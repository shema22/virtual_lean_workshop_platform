...
{
$uid=$_SESSION['samssid'];
$cpassword=md5($_POST['currentpassword']);
$newpassword=md5($_POST['newpassword']);

$sql ="SELECT attendeeID FROM attendees WHERE attendeeID=? and password=?";
$query= $conn->prepare($sql);
$query-> bind_param('ss', $uid, $cpassword);
$query-> execute();
$query-> store_result();

if($query-> num_rows > 0)
{
$con="update attendees set password=? where attendeeID=?";
$chngpwd1 = $conn->prepare($con);
$chngpwd1-> bind_param('ss', $newpassword, $uid);
$chngpwd1->execute();
